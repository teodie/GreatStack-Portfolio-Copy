...
function Navbar({ isDarkMode, setIsDarkMode }: themeToggle) {
  ...
  return (
    <>
      ...
      <nav className={`
    w-full fixed px-5 py-4 flex items-center justify-between z-50 ${isScrolled && "bg-white/50 backdrop-blur-lg shadow-sm dark:bg-dark-theme dark:shadow-white/20"}
    lg:px-8
    xl:px-[8%]
    `}>
        <a href=''>
        ...
        <ul className={`
      hidden items-center gap-6 rounded-full px-12 py-3 ${isScrolled ? "" : "bg-white/50 shadow-sm dark:border dark:border-white/50 dark:bg-transparent"}
      md:flex 
      lg:gap-8
      `}>
        ...
        <div className='flex items-center gap-4'>
          ...
          <a href="#contact"
            className='hidden items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4
          lg:flex font-ovo'>
            Contact
            <Image src={isDarkMode ? assets.arrow_icon_dark : assets.arrow_icon} className='w-3' alt='call' />
          </a>
